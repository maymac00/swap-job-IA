...
def Expert(user, offers):
    def getPunctuation(user, offer):
        puntuation = np.zeros(
    ...
    compatibilitys = []
    for offer in offers:
        compatibilitys.append((getPunctuation(user, offer), offer[5]))
    compatibilitys.sort(key=lambda x: x[0], reverse=True)
    return np.array(compatibilitys)[:, 1].astype(int)
